  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3 md:hidden">
        ...
        {data?.map((t) => (
          <article key={t.id} className="rounded-xl border border-gray-800 bg-gray-900/60 p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <StatusPill active={t.is_active} sub={t.subscription_status} />
            </div>
            <div className="mt-4 flex flex-wrap gap-2">
              {t.is_active ? (
                <button
                  ...
                  className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs font-semibold text-red-300 disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-xs font-semibold text-emerald-300 disabled:opacity-50"
                >
              ...
              <button
                onClick={() => {
                  if (
                    !confirm(
                      `PERMANENTLY delete "${t.name}" and all workspace data?\n\nThis removes the tenant, leads, customers, and member access. Type OK to proceed.`,
                    )
                  )
                    return
                  remove.mutate(t.id)
                ...
                className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-red-900/50 bg-red-950/40 px-3 py-2 text-xs font-semibold text-red-400 disabled:opacity-50"
              >
                  ...
                  <div className="inline-flex flex-wrap items-center justify-end gap-1.5">
                    ...
                    <button
                      onClick={() => {
                        if (
                          !confirm(
                            `PERMANENTLY delete "${t.name}" and all workspace data? Members will lose access. This cannot be undone.`,
                          )
                        )
                          return
                        remove.mutate(t.id)
