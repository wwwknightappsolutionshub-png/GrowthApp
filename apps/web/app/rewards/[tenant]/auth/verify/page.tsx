...
import { rewardsPath, setLoyaltyToken } from '@/lib/loyalty-portal-auth'
...
function VerifyInner({ tenant }: { tenant: string }) {
  ...
  useEffect(() => {
    ...
    loyaltyPortalCustomer
      .verifyMagicLink(slug, token)
      .then(({ data }) => {
        setLoyaltyToken(slug, data.access_token)
        const next = params.get('next')
        router.replace(next ? decodeURIComponent(next) : rewardsPath(slug, 'dashboard'))
      })
      .catch(() => {
  ...
  if (error) {
    return <div className="card mx-auto max-w-sm text-center text-sm text-red-600">{error}</div>
  }

  return <p className="text-center text-sm text-slate-500">Signing you in…</p>
}

export default function RewardsVerifyPage({ params }: { params: { tenant: string } }) {
  return (
