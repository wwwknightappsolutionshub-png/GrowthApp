...
import { isLoyaltyAuthenticated } from '@/lib/loyalty-portal-auth'
...
export function LoyaltyNotificationBell({ tenant }: { tenant: string }) {
  ...
  const authed = isLoyaltyAuthenticated(tenant)

  const refreshUnread = useCallback(async () => {
    if (!authed) return
    try {
      const res = await loyaltyPortalCustomer.unreadCount(tenant)
  ...
  const loadNotifications = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const res = await loyaltyPortalCustomer.listNotifications(tenant)
  ...
  async function markRead(id: string) {
    await loyaltyPortalCustomer.markNotificationRead(tenant, id)
    setItems((prev) => prev.map((n) => (n.id === id ? { ...n, read_at: new Date().toISOString() } : n)))
    setUnread((n) => Math.max(0, n - 1))
  }

  async function markAllRead() {
    await loyaltyPortalCustomer.markAllNotificationsRead(tenant)
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <ul className="divide-y divide-slate-100">
              {items.map((n) => {
                ...
                const href = n.link ? rewardsPath(tenant, n.link) : undefined
                const row = (
