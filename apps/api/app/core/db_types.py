"""
Database type compatibility layer.
Uses native PostgreSQL types when available, falls back to generic types for SQLite.
"""
import uuid

from sqlalchemy import String, types
from sqlalchemy.dialects.postgresql import UUID as PG_UUID, JSONB as PG_JSONB


class CompatibleUUID(types.TypeDecorator):
    """UUID type that works with both PostgreSQL and SQLite."""
    impl = String(36)
    cache_ok = True

    def load_dialect_impl(self, dialect):
        if dialect.name == "postgresql":
            return dialect.type_descriptor(PG_UUID(as_uuid=True))
        return dialect.type_descriptor(String(36))

    def process_bind_param(self, value, dialect):
        if value is None:
            return None
        if dialect.name == "postgresql":
            return value
        return str(value)

    def process_result_value(self, value, dialect):
        if value is None:
            return None
        if isinstance(value, uuid.UUID):
            return value
        return uuid.UUID(value)


class CompatibleJSONB(types.TypeDecorator):
    """JSONB type that works with both PostgreSQL and SQLite."""
    impl = types.JSON
    cache_ok = True

    def load_dialect_impl(self, dialect):
        if dialect.name == "postgresql":
            return dialect.type_descriptor(PG_JSONB())
        return dialect.type_descriptor(types.JSON())


# Export convenience aliases
UUIDType = CompatibleUUID
JSONBType = CompatibleJSONB
